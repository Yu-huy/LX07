    ...
    public class WorkInfo
    {
        ...
        public Model.WorkInfoNo GetModel(int id)
        {
            using (IDbConnection cn = new MySqlConnection(cns))
            {
                ...
                return cn.QueryFirstOrDefault(sql, new { id = id });
            }
        }
        ...
        public int Update(Model.WorkInfo workInfo)
        {
            using (IDbConnection cn = new MySqlConnection(cns))
            {
                string sql = "upadate workinfo set workName=@workName,workPicture=@workPicture,uploadTime=@uploadTime,workIntroduction=@workIntroduction,workVerify=@workVerify," +
                    "userName=@userName,activityId=@activityId,recommend=@recommend,recommendTime=@recommendTime where workId=@workId";
                return cn.ExecuteScalar<int>(sql, workInfo);
            }
        }
        public int UpdateImg(Model.WorkInfo workInfo)
        {
            using (IDbConnection cn = new MySqlConnection(cns))
            {
                string sql = "upadate workinfo set workPicture=@workPicture where workId=@workId";
                if (workInfo.workPicture == null)
                    sql = "upadate workinfo set workIntroduction=@workIntroduction where workId=@workId";
                return cn.ExecuteScalar<int>(sql,workInfo);
            }
        }
        public int UpdateVerify(Model.WorkInfo workInfo)
        {
            using (IDbConnection cn = new MySqlConnection(cns))
            {
                string sql = "upadate workinfo set workVerify=@workVerify where workId=@workId";
                return cn.ExecuteScalar<int>(sql, workInfo);
            }
        }
        ...
        public int UpdateRecommend(Model.WorkInfo workInfo)
        {
            using (IDbConnection cn = new MySqlConnection(cns))
            {
                string sql = "upadate workinfo set recommend=@recommend,recommendTime=@recommendTime where workId=@workId";
